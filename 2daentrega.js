     ...
     $(`#id${productoAgregado.sku}`).click(() => {
        ...
        cart.forEach((item, index) => {
            ...
            if (item.sku == productoAgregado.sku) {
                ...
                console.log(index)
                // si coincide , que deberia coincidir si o si, aplico el splice 
                //que va a borrar una posicion para adelante desde el index que yo le marque
                cart.splice(index, 1)
                console.log(cart)
            }
...
function total() {
    const totalCompra = cart.map(items => items.precio)
...
confirmar.onclick = function () {
    ...
    items.textContent = "";
    $("#items").append(
        `<div> SU COMPRA FUE CONFIRMADA</div><br>
